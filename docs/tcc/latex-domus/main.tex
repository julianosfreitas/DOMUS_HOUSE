...
\usepackage{xurl} % permite quebrar URLs longas (ex.: link do formulário) sem estourar a margem
\usepackage{microtype} % justificação fina (protrusão/expansão) — reduz estouros de margem
...
\AddToShipoutPictureBG{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=north west, inner sep=0, xshift=-10mm, yshift=10mm]
      at (current page.north west) {\includegraphics{timbre/background-nr.png}};
    \node[anchor=north east, inner sep=1.3cm, yshift=5mm]
      at (current page.north east) {\includegraphics{timbre/logo-nr.png}};
  \end{tikzpicture}%
}
...
\input{sections/04-materiais-metodos}
\input{sections/05-desenvolvimento}
\input{sections/06-resultados}
\input{sections/07-consideracoes}

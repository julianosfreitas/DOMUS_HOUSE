...
\begin{tikzpicture}[
  ...
  layer/.style={draw, rounded corners=3pt, align=center, inner sep=5pt,
                minimum width=0.56\textwidth, fill=black!3},
  side/.style={draw, rounded corners=2pt, align=center, inner sep=3pt,
               fill=black!7, font=\scriptsize},
  dev/.style={draw, rounded corners=2pt, align=center, inner sep=3pt,
              fill=black!5, minimum width=42mm, minimum height=12mm},
  arrow/.style={->, very thick},
  note/.style={font=\scriptsize\itshape, fill=white, inner sep=1.5pt}
]
  \node[layer] (pwa)
    {\textbf{Cliente --- PWA (Next.js\,15 / React)}\\
     {\scriptsize instalável no celular (\emph{add to home screen}) · tempo real · tema claro/escuro}};
  \node[layer, below=12mm of pwa] (hub)
    {\textbf{\textit{Hub} --- \emph{back end} NestJS} (Node\,20 · Prisma · PostgreSQL)\\[2pt]
     {\scriptsize \texttt{auth}·\texttt{users}·\texttt{devices}·\texttt{automations}·\texttt{scenes}·\texttt{energy}·\texttt{voice}·\texttt{websocket}}\\[1pt]
     {\scriptsize \textbf{Voz: Whisper (STT) no \emph{hub}} · fila serializada por dispositivo}};
  \node[layer, below=12mm of hub] (adapt)
    {\textbf{Camada de \emph{Adapters}} --- \texttt{DeviceAdapterFactory}\\[2pt]
     {\scriptsize \textbf{TapoAdapter} (KLAP) · \textbf{TuyaAdapter} (LAN\,6668) · \textbf{MockAdapter}}\\
     {\scriptsize (+ Tuya Cloud · Home Assistant --- aditivos)}};
  \node[side, right=5mm of hub] (db) {PostgreSQL\\dados +\\métricas};
  \node[side, left=5mm of adapt] (cloud) {Nuvem\\\emph{só no}\\\emph{comissionar}};
  \node[dev] (lamp) at ([xshift=-28mm,yshift=-16mm]adapt.south)
    {\textbf{Lâmpada Intelbras EWS\,410}\\{\scriptsize Wi-Fi · Tuya}};
  \node[dev] (plug) at ([xshift=28mm,yshift=-16mm]adapt.south)
    {\textbf{Tomada TP-Link Tapo\,P110}\\{\scriptsize Wi-Fi · KLAP · energia}};
  \draw[arrow] (pwa) -- node[note] {REST/JWT · WebSocket} (hub);
  \draw[arrow] (hub) -- node[note] {contrato \texttt{DeviceAdapter}} (adapt);
  \draw[<->, thick] (hub.east) -- (db.west);
  \draw[arrow, dashed] (cloud.east) -- (adapt.west);
  \draw[arrow] (adapt.south) -- (lamp.north);
  \draw[arrow] (adapt.south) -- (plug.north);
  \node[note] at ([yshift=-7.5mm]adapt.south) {LAN 2,4\,GHz --- \emph{local-first}};
\end{tikzpicture}
\caption{Arquitetura em camadas do DOMUS: o PWA fala com o \emph{hub} NestJS (REST/JWT e
WebSocket na mesma origem); todo controle de \emph{hardware} passa pela camada de
\emph{adapters} (\texttt{DeviceAdapterFactory}) sobre a rede local, com a voz processada no
próprio \emph{hub}. A nuvem entra apenas no comissionamento.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{01-voz.png}
\caption{Assistente de voz TUCO no DOMUS: o usuário fala, o Whisper transcreve no
\emph{hub} e o \textit{parser} interpreta a intenção em português, com \textit{earcons}
sinalizando os estados ``ouvindo'', ``processando'' e ``executado''.}
\label{fig:voz-tuco}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{02-dispositivos.png}
\caption{Tela de Dispositivos: cada conexão é uma linha enxuta com estado em tempo real
(a tomada Tapo~P110 física aparece \emph{online}, controlável na rede local).}
\label{fig:dispositivos}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{03-dispositivos-consumo.png}
\caption{Ao abrir um dispositivo, o DOMUS exibe o consumo do aparelho (potência recente e
média, kWh estimado e gráfico) e os controles --- dados lidos diretamente do medidor do
Tapo~P110.}
\label{fig:dispositivos-consumo}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{06-rotinas.png}
\caption{Rotinas e cenas: automações por horário (\emph{cron}) e cenas de um toque
compartilham o mesmo executor de ações (\texttt{ActionsRunner}).}
\label{fig:rotinas}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.42\textwidth]{07-login.png}\hfill
\includegraphics[width=0.42\textwidth]{09-mobile-voz.png}
\caption{À esquerda, a tela de entrada (com login social e o botão ``Entrar como
demonstração''); à direita, o DOMUS instalado como aplicativo no celular (PWA
``adicionar à tela inicial''), responsivo e operando na rede local.}
\label{fig:pwa-mobile}
\end{figure}
